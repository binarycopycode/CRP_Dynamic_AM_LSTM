...
class Env():
    ...
    def find_target_stack(self):
        ...
        #mx_val(batch,stacks)
        mx_val=torch.max(self.x,dim=2)[0]
        #target_stack(batch)
        self.target_stack=torch.argmax(mx_val,dim=1)
    ...
    def _get_step(self, next_node):
        ...
        target_stack_len = torch.gather(stack_len, dim=1, index=self.target_stack[:, None])

        #next_stack_len(batch,1)[i][0]=stack_len[i][next_node[i][0]]
        next_stack_len=torch.gather(stack_len,dim=1,index=next_node)

        #  top_val(batch,max_stacks,1)=x(batch,max_stacks,stack_len(i,j,1))
        #top_ind(batch,max_stacks)-1 ,再用where把空的边0
        top_ind=stack_len-1
        top_ind=torch.where(top_ind>=0,top_ind,0)
        #top_val(batch,max_stacks,1)=x(batch,max_stacks,top_ind(i,j,1))
        top_val=torch.gather(self.x,dim=2,index=top_ind[:,:,None])
        #top_val(batch,max_stacks)
        top_val=top_val.squeeze(-1)
        #target_top_val(batch,1)=top_val(batch,self.target_stack[batch,1])
        target_top_val=torch.gather(top_val,dim=1,index=self.target_stack[:,None])

        #target_ind(batch,1)
        target_ind=target_stack_len-1
        target_ind=torch.where(target_ind>=0,target_ind,0)
        input_index=(torch.arange(self.batch),self.target_stack,target_ind.squeeze(-1))
        self.x=self.x.index_put_(input_index,torch.Tensor([0.]).to(self.device))


        input_index=(torch.arange(self.batch),next_node.squeeze(-1),next_stack_len.squeeze(-1))
        self.x=self.x.index_put_(input_index,target_top_val.squeeze(-1))
    ...
    def create_mask_t1(self):
        ...
        top_val=self.x[:,:,-1]
        #如果最后一列是0，那么这列就是可选的
        mask=torch.where(top_val>0,True,False)
        mask = mask.bool()
